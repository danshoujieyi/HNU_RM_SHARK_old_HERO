...
static void macacum_calc(struct chassis_cmd_msg *cmd, int16_t* out_speed);
static void (*chassis_calc_moto_speed)(struct chassis_cmd_msg *cmd, int16_t* out_speed) = macanum_calc;
static struct chassis_real_speed_t macanum_get_speed(dji_motor_object_t *chassis_motor[4]);
...
void chassis_thread_entry(void *argument)
{
    ...
    for (;;)
    {
        ...
        /* 计算实际速度 */
        macanum_get_speed(chassis_motor);
        /* 更新该线程所有的订阅者 */
        chassis_sub_pull();
...
int TIM_Init(void)
{
 ...
 static struct chassis_real_speed_t macanum_get_speed(dji_motor_object_t *chassis_motor[4])//里程计计算函数。
{
...
static struct chassis_real_speed_t macanum_get_speed(dji_motor_object_t *chassis_motor[4])//里程计计算函数。
{
    ...
    struct chassis_real_speed_t real_speed;
    vw_ch =real_speed.chassis_vw_ch = (wheel_rpm[0] + wheel_rpm[1] + wheel_rpm[2] + wheel_rpm[3]) / (2* wheel_rpm_ratio * (rotate_ratio_f + rotate_ratio_b));
    vy_ch =real_speed.chassis_vy_ch = (wheel_rpm[1] + wheel_rpm[2] - wheel_rpm[0] - wheel_rpm[3]) / (4 * wheel_rpm_ratio);
    vx_ch =real_speed.chassis_vx_ch = (wheel_rpm[0] + wheel_rpm[1] - wheel_rpm[2] - wheel_rpm[3]) / (4 * wheel_rpm_ratio) ;
    vx_gim =real_speed.chassis_vx_gim = real_speed.chassis_vx_ch* cos(angle_hd) - real_speed.chassis_vy_ch* sin(angle_hd);
    vy_gim =real_speed.chassis_vy_gim = real_speed.chassis_vx_ch* sin(angle_hd) + real_speed.chassis_vy_ch* cos(angle_hd);
