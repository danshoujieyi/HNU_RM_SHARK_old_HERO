...
/*----------------------射击状态初始化----------------------------------*/
    ...
    for (;;)
    {
       ...
       /* if (sht_motor[TRIGGER_MOTOR]->measure.real_current>=8000||reverse_cnt!=0)
        ...
        case SHOOT_ONE:
            shoot_motor_ref[RIGHT_FRICTION] = 6500;//摩擦轮常转
            ...
            {
                shoot_motor_ref[TRIGGER_MOTOR]= shoot_motor_ref[TRIGGER_MOTOR] + TRIGGER_MOTOR_60_TO_ANGLE * 19;//M3508的减速比为19:1，因此转轴旋转60度，要在转子的基础上乘19倍
        ...
        case SHOOT_THREE:
            ...
            if (shoot_cmd.trigger_status == TRIGGER_ON)
            {
                shoot_motor_ref[TRIGGER_MOTOR]= shoot_motor_ref[TRIGGER_MOTOR] + 3 * TRIGGER_MOTOR_60_TO_ANGLE * 19;//M3508的减速比为19:1，因此转轴旋转60度，要在转子的基础上乘19倍
                shoot_cmd.trigger_status=TRIGGER_OFF;//扳机归零
            }
            shoot_fdb.shoot_mode=SHOOT_OK;
            break;
        ...
        case SHOOT_COUNTINUE:
            ...
            {
                shoot_motor_ref[TRIGGER_MOTOR] = 550;//自动模式的时候，只用速度环控制拨弹电机
            }
            else if(shoot_cmd.shoot_freq>=5&&shoot_cmd.shoot_freq<8)
            {
                shoot_motor_ref[TRIGGER_MOTOR] = 550;
            }
...
static rt_int16_t motor_control_trigger(dji_motor_measure_t measure){
    ...
    {// shoot_motor_ref[TRIGGER_MOTOR]   reff  pid调试口
